...
#       APP_USER=flask ./control.sh usershell ls /
...
#       APP_USER=flask ./control.sh userexec ls /
...
#       APP_USER=flask ./control.sh duserexec -> run interactive shell inside default CONTAINER
#       APP_USER=flask ./control.sh duserexec foo123 -> run interactive shell inside foo123 CONTAINER
#       APP_USER=flask ./control.sh duserexec flask_123 ls / -> run comand inside foo123 CONTAINER
do_duserexec() {
...
set_tests_command() {
    TESTS_COMMAND="${TESTS_COMMAND:-"py.test --basetemp=. ${@:-tests}"}"
}
...
do_coverage() {
    set_tests_command $@
    do_usershell bash -euxc "\
    export COVERAGE_FILE=coverage_report
    coverage erase
    coverage run --source=src $TESTS_COMMAND
    coverage report"
}
